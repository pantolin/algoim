...
#else
#error \
  "WITH_LAPACK directive is active, then Algoim requires a LAPACKE implementation to compute eigenvalues and SVD factorisations, but a suitable lapacke.h include file was not found; did you forget to specify its include path?"
#endif
...
static real
pythag(const real a, const real b)
{
 ...
 */
static bool
svdcmp(real **a, int nRows, int nCols, real *w, real **vt)
{
...
static void
monomialCoefficients(const real *bernstein, int P, real *monomial)
{
  for (int i = 0; i < P; ++i)
    monomial[i] = 0.0;
...
static void
bernsteinCoefficients(const real *monomial, int P, real *bernstein)
{
  for (int i = 0; i < P; ++i)
    bernstein[i] = 0.0;
 ...
 */
static void
divideByX(const real *alpha, const int P, real *beta)
{
  real tol = 0.0;
  for (int i = 0; i < P; ++i)
...
generalisedEigenvaluesLAPACK(xarray<real, 2> &A, xarray<real, 2> &B, xarray<real, 2> &out)
{
  ...
  const int info =
    LAPACKE_dggevx(LAPACK_ROW_MAJOR, 'B', 'N', 'N', 'N', N, A.data(), N, B.data(), N, alphar, alphai, beta, nullptr, N,
                   nullptr, N, &ilo, &ihi, lscale, rscale, &abnrm, &bbnrm, nullptr, nullptr);
 ...
 */
static void
computeSVD(real *const A, const int m, const int n, real *const U, real *const W, real *const Vt)
{
  assert(0 < m && 0 < n);
