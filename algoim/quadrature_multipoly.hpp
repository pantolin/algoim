...
{
template <int N>
booluarray<N, ALGOIM_M>
mask_driver(const xarray<real, N>& f,
...
template <int N>
booluarray<N, ALGOIM_M>
intersectionMask(const xarray<real, N>& f,
                 const booluarray<N, ALGOIM_M>& fmask,
                 const xarray<real, N>& g,
                 const booluarray<N, ALGOIM_M>& gmask)
{
...
template <int N>
booluarray<N, ALGOIM_M>
nonzeroMask(const xarray<real, N>& f, const booluarray<N, ALGOIM_M>& fmask)
{
...
template <int N>
booluarray<N - 1, ALGOIM_M>
collapseMask(const booluarray<N, ALGOIM_M>& mask, int k)
{
...
template <int N>
bool
maskEmpty(const booluarray<N, ALGOIM_M>& mask)
{
  return mask.none();
}
...
template <int N>
bool
pointWithinMask(const booluarray<N, ALGOIM_M>& mask, const uvector<real, N>& x)
{
  using std::floor;
  uvector<int, N> cell;
  for (int dim = 0; dim < N; ++dim)
...
template <int N>
bool
lineIntersectsMask(const booluarray<N, ALGOIM_M>& mask, const uvector<real, N - 1>& x, int k)
{
  using std::floor;
  if constexpr (N > 1)
  {
...
template <int N>
void
restrictToFace(const xarray<real, N>& a, int k, int side, xarray<real, N - 1>& out)
{
...
template <int N>
booluarray<N - 1, ALGOIM_M>
restrictToFace(const booluarray<N, ALGOIM_M>& a, int k, int side)
{
  assert(0 <= k && k < N && (side == 0 || side == 1));
...
template <typename T>
T
det_qr(xarray<T, 2>& A, int& rank, T tol = 10.0)
{
...
template <int N>
uvector<int, N - 1>
resultantExtent(const uvector<int, N>& p, const uvector<int, N>& q, int dim)
{
  uvector<int, N - 1> ext;
  for (int i = 0; i < N - 1; ++i)
  {
    ...
    // Above 1000, the SVD of Bernstein-Vandermonde matrix fails in double precision.
    ext(i) = std::min(ext(i), 1000);
  }
  return ext;
}
...
template <int N>
uvector<int, N - 1>
discriminantExtent(const uvector<int, N>& p, int dim)
{
  uvector<int, N - 1> ext;
  for (int i = 0; i < N - 1; ++i)
  {
...
template <int N>
bool
resultant_core(const xarray<real, N>& p, const xarray<real, N>* q, int k, xarray<real, N - 1>& out)
{
  assert(0 <= k && k < N);
...
template <int N>
bool
resultant(const xarray<real, N>& p, const xarray<real, N>& q, int k, xarray<real, N - 1>& out)
{
...
template <int N>
bool
discriminant(const xarray<real, N>& p, int k, xarray<real, N - 1>& out)
{
...
template <int N>
void
eliminate_axis(PolySet<N, ALGOIM_M>& phi, int k, PolySet<N - 1, ALGOIM_M>& psi)
{
...
template <int N>
uvector<real, N>
score_estimate(PolySet<N, ALGOIM_M>& phi, uvector<bool, N>& has_disc)
{
