...
template <typename T>
void
evalBernsteinBasis(const T& x, int P, T* out)
{
  assert(P >= 1);
...
template <int N>
real
evalBernsteinPoly(const xarray<real, N>& beta, const uvector<real, N>& x)
{
...
void
bernsteinValueAndDerivative(const real* alpha, int P, const real* binom, real x, real& value, real& deriv)
{
  assert(P > 1);
  real *a, *b;
...
template <int N>
uvector<real, N>
evalBernsteinPolyGradient(const xarray<real, N>& beta, const uvector<real, N>& x)
{
...
template <int N>
void
reverseScaledCoeff(xarray<real, N>& p)
{
...
template <int N>
real
squaredL2norm(const xarray<real, N>& p)
{
...
template <int N>
void
collapseAlongAxis(const xarray<real, N>& beta, const uvector<real, N - 1>& x0, int dim, real* out)
{
  if constexpr (N == 1)
  {
    assert(dim == 0);
...
template <int N>
void
collapseAlongHyperplane(const xarray<real, N>& beta, int dim, real x, xarray<real, N - 1>& out)
{
...
template <int N>
int
uniformSign(const xarray<real, N>& beta)
{
...
template <typename T>
void
bernsteinDerivative(const T* alpha, int P, T* out)
{
  assert(P >= 2);
  for (int i = 0; i < P - 1; ++i)
  {
...
template <int N>
void
bernsteinDerivative(const xarray<real, N>& a, int dim, xarray<real, N>& out)
{
...
template <int N>
void
elevatedDerivative(const xarray<real, N>& a, int dim, xarray<real, N>& out)
{
...
template <int N>
void
deCasteljauLeft(xarray<real, N>& alpha, real tau)
{
  int P = alpha.ext(0);
  for (int i = 1; i < P; ++i)
    for (int j = P - 1; j >= i; --j)
...
template <int N>
void
deCasteljauRight(xarray<real, N>& alpha, real tau)
{
  int P = alpha.ext(0);
  for (int i = 1; i < P; ++i)
    for (int j = 0; j < P - i; ++j)
...
void
deCasteljau(xarray<real, N>& alpha, const real* a, const real* b)
{
  using std::swap;
  using std::abs;
  if constexpr (N == 1 || B)
...
template <int N>
void
deCasteljau(const xarray<real, N>& alpha, const uvector<real, N>& a, const uvector<real, N>& b, xarray<real, N>& out)
{
...
template <int N, bool B = false>
void
bernsteinElevate(const xarray<real, N>& alpha, xarray<real, N>& beta)
{
...
void
lsqr_bidiagonal(real* alpha, real* beta, int P, xarray<real, 2>& b)
{
...
template <int N, bool B = false>
void
bernsteinReduction(xarray<real, N>& alpha, int dim)
{
...
template <int N>
bool
autoReduction(xarray<real, N>& alpha, real tol = 1.0e3 * std::numeric_limits<real>::epsilon(), int dim = 0)
{
  using std::sqrt;
  using std::abs;
...
template <int N>
bool
orthantTestBase(const xarray<real, N>& x, const xarray<real, N>& y, int sign = 0)
{
...
template <int N>
bool
orthantTest(const xarray<real, N>& f, const xarray<real, N>& g)
{
  if (all(f.ext() == g.ext()))
...
inline real
modifiedChebyshevNode(int i, int P)
{
  assert(0 <= i && i < P);
  using std::cos;
...
template <int N, bool B = false>
void
bernsteinInterpolate(const xarray<real, N>& f, real tol, xarray<real, N>& out)
{
...
void
bernsteinInterpolate(F&& f, xarray<real, N>& out)
{
...
void
generalisedEigenvalues(xarray<real, 2>& A, xarray<real, 2>& B, xarray<real, 2>& out)
{
  int N = A.ext(0);
  ...
  real abnrm, bbnrm;
  int ilo, ihi;
  static_assert(std::is_same_v<real, double>, "Algoim's default LAPACK code assumes real == double; a custom "
                                              "generalised eigenvalue solver is required when real != double");
  int info = LAPACKE_dggevx(LAPACK_ROW_MAJOR, 'B', 'N', 'N', 'N', N, A.data(), N, B.data(), N, alphar, alphai, beta,
                            nullptr, N, nullptr, N, &ilo, &ihi, lscale, rscale, &abnrm, &bbnrm, nullptr, nullptr);
...
void
rootsBernsteinPoly(const real* alpha, int P, xarray<real, 2>& out)
{
...
template <typename F>
bool
newtonBisectionSearch(const F& f, real x0, real x1, real tol, int maxsteps, real& root)
{
  using std::abs;
  real f0, f1, dummy;
  f(x0, f0, dummy);
...
int
bernsteinSimpleRoot(const real* alpha, int P, real tol, real& root)
{
  assert(P >= 2);
  using std::abs;
  for (int i = 0; i < P; ++i)
  ...
  bool b = detail::newtonBisectionSearch([=](real x, real& value, real& prime)
                                         { bernsteinValueAndDerivative(alpha, P, binom, x, value, prime); }, 0, 1,
                                         newton_tol, 12, root);
  return b ? 1 : -1;
}
...
// P - 1), or -1 if failed
int
rootsBernsteinPolyFast(const xarray<real, 1>& alpha, real a, real b, int depth, real tol, real* out)
{
...
int
bernsteinUnitIntervalRealRoots_eigenvalue(const real* alpha, int P, real* out)
{
  using std::abs;
...
// if failed, returns -1
int
bernsteinUnitIntervalRealRoots_fast(const real* alpha, int P, real* out)
{
  using std::max;
  using std::abs;
...
int
bernsteinUnitIntervalRealRoots(const real* alpha, int P, real* out)
{
  using std::sqrt;
  if (P == 1)
    return 0;
...
void
sylvesterMatrix(const real* a, int P, const real* b, int Q, xarray<real, 2>& out)
{
...
void
bezoutMatrix(const real* a, const real* b, int P, xarray<real, 2>& out)
{
