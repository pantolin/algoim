...
#endif
  >>>>>>> 2337ace (Bug fix for elevatedDerivate of order 1)
...
>>>>>>> 2337ace (Bug fix for elevatedDerivate of order 1)
  ...
  void elevatedDerivative(const xarray<real, N>& a, int dim, xarray<real, N>& out)
  {
    ...
    for (auto i = a.loop(); ~i; ++i)
    {
      if (i(dim) == 0)
        ...
        out.l(i) = a.m(i.shifted(dim, -1)) * (-i(dim)) + a.l(i) * (2 * i(dim) - P + 1)
                   + a.m(i.shifted(dim, 1)) * (P - 1 - i(dim));
    }
  }
  ...
  template <int N>
  void deCasteljau(const xarray<real, N>& alpha, const uvector<real, N>& a, const uvector<real, N>& b,
                   xarray<real, N>& out)
  {
...
=======
      ...
      int info =
        LAPACKE_dgesvd(LAPACK_ROW_MAJOR, 'A', 'A', P, P, A, P, result.sigma, result.U, P, result.Vt, P, superb);
