...
template <typename F>
bool
newtonBisection(const F& f, real x0, real x1, real tol, int maxsteps, real& root)
{
  using std::abs;
...
struct OneDimRootFind
{
    ...
    f(f), x(x), dim(dim)
  {
    using std::abs;
    real t;
    if (newtonBisection(*this, xmin, xmax, 1e2 * real(std::numeric_limits<real>::epsilon()) * abs(xmax - xmin), 1024,
                        t))
      roots.push_back(t);
  }
...
rootFind(const F& f,
         ...
         int level = 0)
{
  // If the function is known (by the caller) to be monotone or the recursive level is sufficiently deep, call the
  // monotone case.
  ...
  // The function is not known to be monotone on the given interval (although in a lot of practical cases it will be).
  // Use interval arithmetic to place bounds on the function's value and gradient.
  ...
  // Test to see if the function's derivative is bounded away from zero on the interval; if it is, then call the
  // monotone case.
...
template <bool S>
void
determineSigns(bool positiveAbove, int sign, int& bottomSign, int& topSign)
{
  if (S)
  {
...
struct ImplicitIntegral
{
  ...
  ImplicitIntegral(const Phi& phi,
                   F& f,
                   const uvector<bool, N>& free,
                   const std::array<PsiCode<N>, 1 << (N - 1)>& psi_,
                   int psiCount_,
                   const HyperRectangle<real, N>& xrange,
                   int p,
                   int level = 0) :
    phi(phi), f(f), free(free), psi(psi_), psiCount(psiCount_), xrange(xrange), p(p)
  {
    ...
    for (int i = 0; i < psiCount; ++i)
    {
      ...
      if (!directionOkay)
      {
        if (level < 16)
        {
          ...
          ImplicitIntegral<M, N, Phi, F, S>(
            phi, f, free, psi, psiCount, HyperRectangle<real, N>(xrange.min(), set_component(xrange.max(), ind, xmid)),
            p, level + 1);
          ImplicitIntegral<M, N, Phi, F, S>(
            phi, f, free, psi, psiCount, HyperRectangle<real, N>(set_component(xrange.min(), ind, xmid), xrange.max()),
            p, level + 1);
          return;
        }
        else
    ...
    ImplicitIntegral<M - 1, N, Phi, ImplicitIntegral<M, N, Phi, F, S>, false>(
      phi, *this, set_component(free, e0, false), newPsi, newPsiCount, xrange, p);
  }
};
...
{
  ImplicitIntegral(const Phi&,
                   F&,
                   const uvector<bool, N>&,
                   const std::array<PsiCode<N>, 1 << (N - 1)>&,
                   int,
                   const HyperRectangle<real, N>&,
                   int)
  {
  }
};
...
void
outputQuadratureRuleAsVtpXML(const QuadratureRule<N>& q, T& stream)
{
  ...
  stream << "<PolyData>\n";
  stream << "<Piece NumberOfPoints=\"" << q.nodes.size() << "\" NumberOfVerts=\"" << q.nodes.size()
...
QuadratureRule<N>
quadGen(const F& phi, const HyperRectangle<real, N>& xrange, int dim, int side, int qo)
{
  QuadratureRule<N> q;
  std::array<PsiCode<N>, 1 << (N - 1)> psi;
